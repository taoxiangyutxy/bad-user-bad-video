node {
    ...
    stage('拉取代码') {
        dir("/var/lib/jenkins/workspace/ttt-one-admin") {
            ...
            sh "pwd"
            def res = checkout([$class: 'GitSCM', branches: [[name: '*/master']], extensions: [], userRemoteConfigs: [[credentialsId: '627f5932-ac94-4cdb-b592-003b56076503', url: 'https://gitee.com/yyyyxt/ttt-one.git']]])
            sh "echo ${res}"
    ...
    stage('移动至工作目录') {
        dir("/var/lib/jenkins/workspace/ttt-one-admin") {
            sh "cp -f ./${CURRENT_JOB}/target/${CURRENT_JOB}.jar /mnt/sdc/project/tttServer/"
        }
    }
    stage('启动当前模块') { //怎么启动？
